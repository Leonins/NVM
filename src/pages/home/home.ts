	...
	mapDisplay() {
		...
		map.addEventListener(GoogleMapsEvent.MAP_READY, function () {
			...
			function checkInPolygon(currentLocation, polygons) {
				...
				console.log(polygons);
				polygons.forEach(function (polyg){
					checkInside.push({'check':Poly.containsLocation(currentLocation, polyg.points)});
				});
				console.log(checkInside);
				checkInside.forEach(function (checkgeo) {
					console.log('ga asdasda '+checkgeo.check);
					if(checkgeo.check == true){
						alert("error!");
					}
				});
				// checkInside[k] =  Poly.containsLocation(currentLocation, polygons.points);
				// console.log('ga tarug eskwela '+checkInside[k]);
